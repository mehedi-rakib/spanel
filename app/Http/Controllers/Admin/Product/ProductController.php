...
class ProductController extends BaseController
{
    ...
    public function add(ProductAddRequest $request, ProductService $service, StockHistoryService $stockHistoryService): JsonResponse|RedirectResponse
    {
        if ($request->ajax()) {
        ...
        if ($savedProduct->product_type == 'physical') {
            $stockHistoryService->recordInitial(
                productId: $savedProduct->id,
                quantity: (int)$savedProduct->current_stock,
                note: translate('initial_stock_from_product_creation')
            );
        }
    ...
    public function updateQuantity(Request $request, StockHistoryService $stockHistoryService): RedirectResponse
    {
        $variations = [];
        ...
        if ($stockCount >= 0) {
            ...
            $stockHistoryService->recordAbsolute(
                productId: (int)$request['product_id'],
                type: StockHistoryService::TYPE_ADJUSTMENT,
                newStockValue: (int)$stockCount,
                note: translate('manual_stock_update_from_product_list')
            );
            $this->productRepo->updateByParams(params: ['id' => $request['product_id']], data: ['variation' => json_encode($variations)]);
    ...
    public function importBulkProduct(Request $request, ProductService $service, StockHistoryService $stockHistoryService): RedirectResponse
    {
        ...
        $slugs = array_column($dataArray['products'], 'slug');
        $importedProducts = \App\Models\Product::whereIn('slug', $slugs)->get(['id', 'current_stock']);
        foreach ($importedProducts as $importedProduct) {
            $stockHistoryService->recordInitial(
                productId: $importedProduct->id,
                quantity: (int)$importedProduct->current_stock,
                note: translate('initial_stock_from_bulk_import')
            );
        }
    ...
    public function getBulkEditView(ProductBulkEditService $service): View
    {
        $fields = $service->editableFields();
        $translatableFields = $service->translatableFields();
        $languages = $service->languages();
        $categories = $this->categoryRepo->getListWhere(filters: ['position' => 0], dataLimit: 'all');
        $brands = $this->brandRepo->getListWhere(dataLimit: 'all');
        return view(Product::BULK_EDIT[VIEW], compact('fields', 'translatableFields', 'languages', 'categories', 'brands'));
    }

    public function exportBulkEditData(Request $request, ProductBulkEditService $service): StreamedResponse
    {
        $request->validate([
    ...
    {
        $result = $service->processBulkEditUpload(request: $request);
        if (!$result['status']) {
    ...
    public function submitPurchase(Request $request, StockHistoryService $stockHistoryService): RedirectResponse
    {
        ...
        foreach ($request['product_id'] as $index => $productId) {
            ...
            $unitCost = (isset($request['unit_cost'][$index]) && $request['unit_cost'][$index] !== '')
                ? (float)$request['unit_cost'][$index] : null;

            $stockHistoryService->recordChange(
            ...
            if ($unitCost !== null) {
                $this->productRepo->update(id: $productId, data: ['purchase_price' => $unitCost]);
            }
            $count++;
        }
