...
class ProductRepository implements ProductRepositoryInterface
{
    ...
    public function cursorWhere(array $filters = [], array $relations = []): \Illuminate\Support\LazyCollection
    {
        ...
        return $this->product->with($relations)
            ->when($searchValue, function ($query) use ($filters, $searchValue) {
                ...
                return $query->where(function ($query) use ($filters, $searchValue, $productIds) {
                    $query->where('name', 'like', "%{$searchValue}%")
                        ->orWhereIn('id', $productIds);
                    if (!empty($filters['code'])) {
                        $query->orWhere('code', 'like', '%' . $filters['code'] . '%');
                    }
                });
            })
